...
frappe.pages["per-piece-entry"].on_page_load = function (wrapper) {
...
window.per_piece_payroll.PerPieceEntryNativePage = class PerPieceEntryNativePage {
	constructor(wrapper, options) {
		...
		this.styleId = "pp-native-entry-style";
		this.load();
	}

	async load() {
		this.$root = $(this.wrapper).find(".layout-main-section");
		this.$root.html(`<div class="text-muted">${__("Loading Per Piece Entry...")}</div>`);
	...
	render(html) {
		...
		window.PER_PIECE_BOOT = {
			workspace: this.options.workspace || "entry",
			tab: this.options.tab || "data_entry",
		};
	...
	applyWorkspaceState(attempt) {
		...
		const entryWorkspace = document.getElementById("pp-workspace-entry");
		if (!entryWorkspace) {
	...
	injectWorkspaceStyle() {
		...
		style.textContent = `
			#pp-workspace-reporting { display: none !important; }
			.pp-tab[data-workspace='reporting'] { display: none !important; }
		`;
